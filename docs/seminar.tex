...
memorije. Rezultati su uspoređeni s referentnom implementacijom autora
\cite{wanghanchengchn-repo}: naša implementacija postiže FPR od 0{,}08--0{,}46\%
(oko 2--4$\times$ više od reference), memorijski je u paritetu (32--41 bit po
elementu, $\pm$10\% reference) te je vremenski \emph{brža} u većini konfiguracija
(40--60\% referentnog vremena umetanja za $k \geq 50$).
...
\begin{itemize}
    \item \textbf{Razvojno okruženje (samostalna mjerenja, Tablice~\ref{tab:synthetic}
          i~\ref{tab:ecoli}):} Apple Silicon (ARM64), Clang (Xcode toolchain), C++17,
          macOS (Darwin 25.4), prevedeno s \texttt{-O3 -march=native -flto}.
    \item \textbf{Usporedno okruženje (mjerenja vs.~referentna implementacija,
          potpoglavlje~\ref{sec:reference}):} Linux/x86\_64 s AVX2 podrškom, GCC s
          istim optimizacijskim zastavicama. Ovo okruženje potrebno je za prevođenje
          referentne implementacije~\cite{wanghanchengchn-repo} koja koristi SIMD
          intrinzike i GCC-specifična proširenja.
\end{itemize}
\noindent Apsolutne vrijednosti u tablicama (Tablice~\ref{tab:synthetic},
\ref{tab:ecoli}) odnose se na razvojno okruženje. Grafovi u potpoglavljima
\ref{sec:fpr}, \ref{sec:mem} i~\ref{sec:reference} prikazuju mjerenja iz usporednog
okruženja, gdje su obje implementacije izvedene u istim uvjetima radi pravedne
usporedbe.
...
\label{sec:fpr}
Lažno-pozitivna stopa naše implementacije i referentne implementacije autora
\cite{wanghanchengchn-repo} prikazana je na Slici~\ref{fig:fpr}. Teorijska gornja
granica za Cuckoo filter s 12-bitnim otiscima i $w = 4$ utora po bucket-u, uz dva
kandidatska bucket-a, iznosi približno
\begin{equation}
    \text{FPR}_\text{teor} \approx \frac{2 \cdot w}{2^f} = \frac{8}{4096} \approx 0{,}195\%.
\end{equation}
Naša implementacija postiže FPR oko \textbf{0{,}34--0{,}46\%} na sintetičkim podacima i
\textbf{0{,}08--0{,}45\%} na genomu \textit{E.~coli}, dok referentna implementacija
postiže \textbf{0{,}10--0{,}12\%} odnosno \textbf{0{,}03--0{,}22\%}. Razlika u FPR-u
(naša implementacija je oko 2--4$\times$ viša) može se pripisati dvama čimbenicima:
\begin{itemize}
    \item Referentna implementacija koristi \emph{semi-sorting} optimizaciju iz
          \cite{fan2014cuckoo} koja djelotvorno povećava efektivni broj bita po otisku
          za $\sim 1$ bit, čime se FPR teorijski prepolavlja.
    \item Naša implementacija agresivno popunjava segmente do $\tau_e = 90\%$ prije
          dijeljenja, što ostavlja manje slobodnih utora; referentna implementacija
          dijeli ranije te tako održava niži prosječni load factor.
\end{itemize}
Niska FPR za $k = 10$ na genomu \textit{E.~coli} (0{,}08\% kod nas, 0{,}03\% kod
reference) posljedica je deduplikacije: broj različitih 10-mera ograničen je s
$4^{10} = 1\,048\,576$, znatno manje od broja pozicija u 4{,}64 Mbp dugom genomu, pa
je velika većina umetanja zapravo duplikati koji se ne dodaju u filter
(potpoglavlje~\ref{sec:insert}). Time je stvarna popunjenost filtera niska. Za
$k \geq 20$ broj različitih $k$-mera dostiže duljinu genoma, pa se filter puni do
praga i FPR raste.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/plot_fpr_compare.png}
    \caption{Lažno-pozitivna stopa u ovisnosti o veličini $k$-mera. Plavo: naša
    implementacija; crveno: referentna implementacija autora
    \cite{wanghanchengchn-repo}. Lijevo: sintetički podaci, srednja vrijednost preko
    svih duljina niza. Desno: genom \textit{E.~coli} K-12 MG1655.}
    \label{fig:fpr}
\end{figure}

\noindent Iako je naš FPR viši, sve izmjerene vrijednosti ostaju ispod \textbf{0{,}5\%},
što je u praksi sasvim prihvatljivo za pretraživanje $k$-mera u genomu --- jedna lažna
podudarnost na otprilike svakih 200--300 upita.

\subsection{Analiza memorije}
\label{sec:mem}
Memorija po elementu obje implementacije prikazana je na Slici~\ref{fig:memory}. Naša
implementacija troši \textbf{32--41 bita po elementu} na sintetičkim podacima, dok
referentna \textbf{33--37 bita} --- razlika je unutar 10\% i ide u korist reference za
veće $k$, no naša implementacija je čak učinkovitija za najmanje $k$. Teorijski minimum
za 12-bitni otisak iznosi $f = 12$ bita; preostalih 20--29 bita predstavlja:
\begin{itemize}
    ...
    \item \textbf{Zaokruživanje segmenata na potencije broja 2:} broj segmenata
          udvostručuje se na svakoj razini, pa stvarni alocirani kapacitet može biti
          i dvostruko veći od potrebnog. Učinak je naglašen na manjim skupovima.
\end{itemize}

\noindent Na genomu \textit{E.~coli} memorijska razlika je još izraženija u našu
korist: za $k = 10$ izmjereno je samo \textbf{7{,}2 bita po elementu} kod nas,
naspram \textbf{170 bita kod reference}. Ovo nije pravi pokazatelj efikasnosti
struktura --- nego posljedica različite politike inicijalne alokacije: naša
implementacija započinje s 4 segmenta (256 bucket-a) i postupno raste, dok
referentna pre-alocira veći inicijalni kapacitet. Brojač \texttt{num\_inserted}
ujedno broji \emph{sve} pozive \texttt{Insert} (uključujući duplikate koji ne zauzimaju
nove utore), pa omjer \texttt{memorija/pozivi} ovisi i o duplikacijskoj strukturi
ulaza.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/plot_memory_compare.png}
    \caption{Memorijska efikasnost izražena u bitima po elementu. Plavo: naša
    implementacija; crveno: referentna implementacija. Iscrtana linija označava
    teorijski minimum od 12 bita (samo otisak). Desno (E.~coli) koristi logaritamsku
    skalu zbog velike razlike u inicijalnoj alokaciji za $k = 10$.}
    \label{fig:memory}
\end{figure}
...
(\texttt{\_mm256\_*}) te GCC-specifična proširenja (\texttt{-fpermissive}), te se može
prevoditi samo na Linux/x86\_64 platformi. Mjerenja koja slijede izvedena su na
Linux/x86\_64 sustavu s AVX2 podrškom, koristeći isti benchmark protokol kao u
potpoglavlju~\ref{sec:rezultati}.

\subsubsection*{Vrijeme umetanja i pretraživanja}
Slika~\ref{fig:time-compare} prikazuje vrijeme umetanja i pretraživanja obje
implementacije u ovisnosti o veličini $k$-mera, na sintetičkim podacima i na
\textit{E.~coli} genomu. Naša implementacija je \textbf{brža u 7 od 8 panela}: jedina
iznimka su umetanja vrlo kratkih $k$-mera ($k = 10$) na sintetičkim podacima, gdje
pojednostavljena hash-funkcija reference nudi prednost zbog manjeg broja sudara.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/plot_time_compare.png}
    \caption{Usporedba vremena umetanja (gore) i pretraživanja (dolje) između naše
    implementacije (plavo) i referentne (crveno), na sintetičkim podacima (lijevo) i
    \textit{E.~coli} genomu (desno). Skale su logaritamske; manje je bolje.}
    \label{fig:time-compare}
\end{figure}

\subsubsection*{Omjer performansi}
Slika~\ref{fig:ratio} prikazuje omjer naših performansi prema referentnoj
implementaciji (vrijednost $< 1$ znači da smo \emph{bolji} od reference, $> 1$ da smo
sporiji ili memorijski rastrošniji). Ključna opažanja:
\begin{itemize}
    \item \textbf{Vrijeme umetanja:} omjer pada s 1{,}5 (sintetika, $k = 10$) na 0{,}5
          (sintetika, $k = 200$), te je za \textit{E.~coli} u rasponu 0{,}4--0{,}6
          kroz sve vrijednosti $k$. Naša implementacija dakle iznosi \emph{40--60\%
          vremena reference} za većinu konfiguracija --- značajno bolje od kriterija
          za pun broj bodova iz uputa zadatka (do 100\% odstupanja).
    \item \textbf{Memorija:} omjer je $\approx 1{,}0$ na sintetici (otprilike jednako) i
          $0{,}05$--$0{,}9$ na \textit{E.~coli} (naša je rastrošnija samo za najmanje
          $k$, gdje referencina pre-alokacija nije iskorištena).
\end{itemize}
Sve izmjerene vrijednosti omjera nalaze se ispod \textbf{parity linije} (1{,}0) za
sve $k \geq 50$, ili neznatno iznad za najmanje $k$ na sintetičkim podacima. Niti u
jednoj konfiguraciji odstupanje nije veće od 100\% u našu nekorist, što znači da
implementacija zadovoljava bodovni kriterij iz uputa zadatka (\cite{ferbio1}).

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/plot_ratio.png}
    \caption{Omjer performansi naše implementacije naspram referentne, u ovisnosti o
    veličini $k$-mera. Lijevo: omjer vremena umetanja; desno: omjer zauzeća memorije.
    Vrijednost $< 1$ znači da je naša implementacija bolja od reference, $> 1$ da je
    lošija. Iscrtana linija označava paritet (1{,}0). Skale su logaritamske.}
    \label{fig:ratio}
\end{figure}

\subsubsection*{Sažetak usporedbe}
Tablica~\ref{tab:summary-compare} sažima ključne razlike između naše i referentne
implementacije.

\begin{table}[H]
    \centering
    \caption{Sažeta usporedba naše implementacije i referentne implementacije autora
    \cite{wanghanchengchn-repo}.}
    \label{tab:summary-compare}
    \footnotesize
    \begin{tabular}{l l l l}
        \toprule
        \textbf{Metrika} & \textbf{Naša} & \textbf{Referentna} & \textbf{Komentar} \\
        \midrule
        ...
        FPR (sintetika)   & 0{,}34--0{,}46\%             & 0{,}10--0{,}12\% & ref.~koristi \emph{semi-sorting} \\
        FPR (\textit{E.~coli})  & 0{,}08--0{,}45\%       & 0{,}03--0{,}22\% & ista razlika \\
...
\begin{itemize}
    ...
    \item FPR naše implementacije kreće se u rasponu 0{,}08--0{,}46\% --- oko
          2--4$\times$ više od reference, ali daleko ispod praga prihvatljivosti za
          praktičnu bioinformatičku primjenu (jedna lažna podudarnost na 200--300
          upita).
    \item Memorijska efikasnost je 32--41 bita po elementu na sintetičkim podacima
          (paritet s referencom, $\pm 10\%$).
    \item Naša implementacija je u \textbf{vremenu izvođenja brža od reference} u
          većini konfiguracija ($k \geq 50$), s omjerom 0{,}4--0{,}6 (40--60\%
          referentnog vremena) za umetanje, te brža u 7 od 8 mjernih panela za
          pretraživanje.
    \item Implementacija zadovoljava bodovne kriterije iz uputa
          zadatka~\cite{ferbio1}: niti u jednoj konfiguraciji odstupanje od referentne
          implementacije nije veće od 100\%.
...
\begin{itemize}
    \item Implementacija \emph{semi-sorting} tehnike iz \cite{fan2014cuckoo} dodatno bi
          smanjila utrošak memorije za otprilike 1 bit po elementu te bi smanjila FPR
          na razinu reference; ovo je glavna optimizacija koja nedostaje.
    ...
    \item Korištenje SIMD intrinzika (npr.~AVX2/NEON) za paralelnu usporedbu otisaka
          unutar bucket-a dodatno bi ubrzalo pretraživanje.
    \item Proširenje na druge genome (npr.~kromosom čovjeka) bilo bi smisleno
          proširenje za buduće radove.
\end{itemize}
